...
iter.pickup <- args[3]
mcmc.pickup.file <- paste0("./inits/mcmc_samples_rho1_", rho.text[1], "_rho2_", rho.text[2], "_iter_", iter.pickup, ".RDS")
...
for(iter in (iter.pickup+1):end.iter){
  ...
  if(iter%%print.freq == 0 & iter > burn) print(paste0(c(rho.text[1], rho.text[2], "burn-iter", iter), collapse = "-"))
  
}

print(paste0(c(rho.text[1], rho.text[2], "iter", iter), collapse = "-"))
...
saveRDS(mcmc.state, paste0("./burnin/mcmc_samples_rho1_", rho.text[1] ,"_rho2_", rho.text[2], "_iter_", iter,".RDS"))
