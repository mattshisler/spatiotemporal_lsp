...
rho <- c(0.99, 0.99)
mcmc.pickup.file <- paste0("/Users/matthewshisler/BLSP_Oct2024/local_results/inits/", 
"mcmc_samples_rho1_", rho[1]*100, "_rho2_", rho[2]*100, "_iter_", 2, ".RDS")
mcmc.pickup <- readRDS(mcmc.pickup.file)

# parameters to vary for different runs.
iter.pickup <- args[3]
rho <- c(args[1]/100, args[2]/100)
...
for(iter in (iter.pickup+1):(iter.pickup + n.iters)){
  ...
  if(iter%%5 == 0 & iter > burn) print(iter)
   ...
   if(iter%%thin == 0 & iter > burn){
     ...
     saveRDS(mcmc.state, paste0("./rho1_", args[1], "_rho2_", args[2], "/mcmc_samples_rho1_", args[1] ,"_rho2_", args[2], "_iter_", iter,".RDS"))
   }
